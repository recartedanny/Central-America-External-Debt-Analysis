...
-- NOTE: Use of ranking to separate make a tier list of the years then i take the first on every year, in this way i can have more than the first 
...
-- IMPORTANT: Costa Rica and Panama excluded — no coverage for this indicator in World Bank database
...
-- =============================================
	...
	with BaseYear as(
		...
		GrowingRate as(
			select b.country_value as c_value,(((f.value -b.value)/nullif(b.value,0))*100) as growing_rate
			from BaseYear b INNER JOIN FinalYear f on b.country_value=f.country_value WHERE b.value IS NOT NULL AND f.value IS NOT NULL
		)
...
-- =============================================
	...
	SELECT country_value, type_of_indicator, COUNT(value) as years_with_data
FROM CENTRAL_AMERICA_DEBT
WHERE country_id IN ('CR', 'PA')
AND value IS NOT NULL
GROUP BY country_value, type_of_indicator
ORDER BY country_value, years_with_data DESC;